...
/// input text and try to select just entered content
#[tauri::command]
pub fn enter_text(text: String) -> Result<(), AppError> {
    ...
    // use Enigo to input text
    let mut enigo_guard = ENIGO.lock()?;
    let enigo = enigo_guard.as_mut()?;
    enigo.text(&text)?;

    // if cursor position is editable, try to select just entered text
    if platform::is_cursor_editable()? {
        ...
        // first try using native API to select output text
