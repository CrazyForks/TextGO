...
pub fn get_selected_text() -> Result<String, AppError> {
    unsafe {
        ...
        let automation: IUIAutomation = CoCreateInstance(&CUIAutomation, None, CLSCTX_ALL)
            .map_err(|e| {
                CoUninitialize();
                format!("Failed to create UI Automation instance: {}", e)
            })?;

        // 获取当前焦点元素
        let focused_el: IUIAutomationElement = automation.GetFocusedElement()
            .map_err(|e| {
                CoUninitialize();
        ...
        // 获取 Text Pattern
        let pattern = focused_el.GetCurrentPattern(UIA_TextPatternId)
            .map_err(|_| {
                CoUninitialize();
                "Element does not support text pattern"
            })?;

        let text_pattern = pattern.cast::<IUIAutomationTextPattern>()
            .map_err(|_| {
                CoUninitialize();
                "Failed to cast to text pattern"
            })?;
        ...
        let length = selection_array.Length()
            .map_err(|_| {
                CoUninitialize();
                "Failed to get selection array length"
            })?;
        ...
        let range = selection_array.GetElement(0)
            .map_err(|_| {
                CoUninitialize();
                "Failed to get selection range element"
            })?;

        let text = range.GetText(-1)
            .map_err(|_| {
                CoUninitialize();
                "Failed to get text from selection range"
            })?;
